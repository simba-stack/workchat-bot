...
async def _open_lk_form_for_client(bot, params: dict) -> str:
    ...
    # 1) Найти owner для этого work_chat, или создать (авто «+партнер»)
    owner = _find_owner_for_work_chat(chat_id)
    if not owner:
        # SIMBA v2 автоматика: клиент этой беседы = partner-owner.
        # Регистрируем его без ручного «+партнер @username».
        try:
            existing = crm_storage.find_crm_owner_by_tg(int(client_id or 0)) if client_id else None
            if existing:
                owner_id = existing["owner_id"]
                # Обновляем work_chat_id — привязываем к текущей беседе
                await crm_storage.update_crm_owner(
                    owner_id, work_chat_id=int(chat_id),
                )
                owner = dict(existing, work_chat_id=int(chat_id))
                logger.info(
                    "[crm-v2 auto-partner] existing owner=%s re-bound to chat=%s",
                    owner_id, chat_id,
                )
            elif client_id:
                owner_id = await crm_storage.add_crm_owner(
                    ...
                    name=client_uname or f"client_{client_id}",
                    work_chat_id=int(chat_id),
                )
                owner = {"owner_id": owner_id, "tg_user_id": int(client_id),
                         "username": client_uname or "", "work_chat_id": int(chat_id)}
                logger.info(
                    "[crm-v2 auto-partner] created owner=%s tg=%s @%s chat=%s",
                    owner_id, client_id, client_uname, chat_id,
                )
            else:
                await bot.send_message(
                    chat_id,
                    "⚠️ Не могу определить клиента этого чата. Оператор скоро подключится.",
                )
                return f"⚠️ open_lk_form: no client_id for chat={chat_id}"
            # Регистрируем chat → owner в crm_chats (иначе /clients в чате не сработает)
            try:
                await crm_storage.register_crm_chat(
                    chat_id=int(chat_id), owner_id=owner_id,
                    is_admin=False, is_password=False, is_otr=False,
                )
            except Exception as _rce:
                logger.warning("[crm-v2 auto-partner] register_crm_chat failed: %s", _rce)
        except Exception as e:
            logger.exception("[crm-v2 auto-partner] failed: %s", e)
            try:
                await bot.send_message(
                    chat_id,
                    "⚠️ Ошибка привязки партнёра. Оператор скоро подключится.",
                )
            except Exception:
                pass
            return f"⚠️ open_lk_form: auto-partner failed: {e}"
    owner_id = owner["owner_id"]
...
async def _payment_reminder_loop(bot):
    ...
    триггеры для notifications (payouts pending >24h, outkup >2h, margin <0)."""
    await asyncio.sleep(30)
    while True:
        try:
        ...
        try:
            await _notification_triggers_tick(bot)
        except Exception as e:
            logger.warning("notification triggers tick failed: %s", e)
        try:
...
async def _notify_owner_tg(bot, text: str, dedup_key: str = None):
    """Шлёт уведомление owner-у в TG личку через @PrideCONTROLE_bot.
    Owner ID из env TRON_OWNER_TG_ID или config.ADMIN_ID.
    Также пишет в storage.notifications для JARVIS bell."""
    # Сначала — в JARVIS-очередь
    try:
        await crm_storage.add_notification(
            type="warning", text=text, dedup_key=dedup_key,
        )
    except Exception as e:
        logger.warning("[notify_owner] storage add failed: %s", e)
    # Затем — в TG личку owner-а
    owner_id = 0
    try:
        owner_id = int(os.environ.get("TRON_OWNER_TG_ID", "0"))
    except Exception:
        pass
    if not owner_id:
        try:
            owner_id = int(config.ADMIN_ID or 0)
        except Exception:
            pass
    if not owner_id:
        return
    try:
        ...
        logger.warning("[notify_owner] TG send to %s failed: %s", owner_id, e)


async def _notification_triggers_tick(bot):
    """Запускается каждые _PAYMENT_LOOP_INTERVAL сек. Проверяет:
      • payouts queue: записи >24h без status=done → notify
      • outkup_orders: pending >2h → notify
      • margin за сегодня <0 → notify (1 раз в день)
    """
    ...
    # 1) Payouts >24h
    try:
        for queue_name in ("pending_pop", "ready_to_release", "pending_payout"):
            for p in (crm_storage.list_payouts(queue_name) or []):
                ts = float(p.get("ts") or p.get("created_at") or 0)
                if not ts:
                    continue
                age_hours = (now - ts) / 3600
                if age_hours >= 24 and p.get("status") not in ("done", "paid", "released"):
                    pid = p.get("id") or p.get("card_id") or "?"
                    dedup = f"payout_stale:{queue_name}:{pid}"
                    await _notify_owner_tg(
                        bot,
                        f"⏰ <b>Выплата ждёт >24ч</b>\nQueue: {queue_name}\nID: {pid}\n"
                        f"Возраст: {int(age_hours)}ч. Проверь в дашборде → Выплаты.",
                        dedup_key=dedup,
                    )
    except Exception as e:
        logger.warning("[notif] payouts check failed: %s", e)

    # 2) Outkup pending >2h
    try:
        outkup_orders = crm_storage.list_outkup_orders() or {}
        for oid, o in outkup_orders.items():
            if o.get("status") not in ("pending_confirm", "awaiting_payment"):
                continue
            ...
            if age_hours >= 2:
                ...
                dedup = f"outkup_stale:{oid}"
                await _notify_owner_tg(
                    bot,
                    f"💱 <b>Откуп ждёт >2ч</b>\nID: {oid}\nСумма: {amount} ₽\n"
                    f"Возраст: {int(age_hours)}ч. Откупщики должны обработать.",
                    dedup_key=dedup,
                )
    except Exception as e:
    ...
    try:
        ...
        summary = crm_storage.get_accounting_summary(date_from=today_start, date_to=now)
        margin = float(summary.get("margin_usdt") or 0)
        if margin < -50:  # запас от шума
            today_str = _time.strftime("%Y-%m-%d", _time.gmtime(now))
            dedup = f"margin_negative:{today_str}"
            await _notify_owner_tg(
                bot,
                f"📉 <b>Маржа за сегодня отрицательная</b>\nМаржа: {margin:.2f}$\n"
                f"Проверь Бухгалтерию.",
                dedup_key=dedup,
            )
    except Exception as e:
        logger.warning("[notif] margin check failed: %s", e)
...
# =====================================================================
# /payout — авто-выплаты работникам USDT TRC20 через Tron mainnet
# (требует TRON_PRIVATE_KEY + TRON_HOT_WALLET_ADDRESS в env)
# =====================================================================

import re as _re_payout

# {payout_request_id: {to_address, amount_usdt, worker_username, reason, requester_id}}
_pending_payouts = {}
_pending_payout_seq = [0]


def _next_payout_req_id() -> str:
    _pending_payout_seq[0] += 1
    return f"pr{int(time.time()*1000)%1000000}{_pending_payout_seq[0]}"


@router.message(Command("set_usdt"))
async def cmd_set_usdt(message: Message):
    """Owner: установить USDT TRC20 адрес работника. /set_usdt @vasya TR7Nx..."""
    if not is_owner(message.from_user.id):
        return
    parts = (message.text or "").split(maxsplit=2)
    if len(parts) < 3:
        await message.reply(
            "Использование: <code>/set_usdt @username TR7N...</code>\n"
            "Адрес сохранится в worker_roles, после этого можно слать /payout."
        )
        return
    ...
    try:
        ...
        if not validate_tron_address(address):
            await message.reply(f"❌ Невалидный Tron-адрес: <code>{address}</code>")
            return
    except Exception as e:
        await message.reply(f"❌ tron_payouts недоступен: {e}")
        return
    await crm_storage.set_worker_usdt_address(username, address)
    await message.reply(
        f"✅ USDT TRC20 адрес работника <b>@{username}</b> сохранён:\n"
        f"<code>{address}</code>\n\nТеперь можно слать /payout @{username} 100 reason"
    )


@router.message(Command("payout"))
async def cmd_payout(message: Message):
    """Owner: /payout @username 100 reason — авто-выплата USDT TRC20."""
    if not is_owner(message.from_user.id):
        return
    text = (message.text or "").strip()
    # /payout @user 100 [reason ...]
    m = _re_payout.match(r"^/payout\s+@?(\w+)\s+([\d.,]+)(?:\s+(.+))?$", text)
    if not m:
        await message.reply(
            "Использование: <code>/payout @username 100 [причина]</code>\n"
            "Пример: <code>/payout @vasya 250 зарплата за май</code>"
        )
        return
    ...
    try:
        ...
        await message.reply("❌ Невалидная сумма")
        return
    reason = (m.group(3) or "salary").strip()

    # Проверим что есть USDT адрес
    address = crm_storage.get_worker_usdt_address(username)
    if not address:
        await message.reply(
            f"❌ Нет USDT адреса для <b>@{username}</b>.\n\n"
            f"Сначала: <code>/set_usdt @{username} TR7N...</code>"
        )
        return

    # Проверка configured tron_payouts
    try:
        from tron_payouts import is_configured, get_hot_wallet_balance
        ...
        if bal.get("error"):
            await message.reply(f"❌ Tron-balance check failed: {bal.get('error')}")
            return
    except Exception as e:
        await message.reply(f"❌ tron_payouts ошибка: {e}")
        return

    # Регистрируем pending запрос + кнопки confirm/cancel
    pid = _next_payout_req_id()
    _pending_payouts[pid] = {
        "to_address": address,
        "amount_usdt": amount,
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Подтвердить выплату", callback_data=f"pay_ok:{pid}"),
        InlineKeyboardButton(text="❌ Отмена", callback_data=f"pay_no:{pid}"),
    ]])
    await message.reply(
        f"💰 <b>Подтверди выплату:</b>\n\n"
        f"Получатель: <b>@{username}</b>\n"
        f"Адрес: <code>{address}</code>\n"
        f"Сумма: <b>{amount} USDT</b>\n"
        f"Причина: <i>{reason}</i>\n\n"
        f"💼 Баланс hot-wallet: <b>{bal.get('usdt', 0):.2f} USDT</b> | "
        f"{bal.get('trx', 0):.2f} TRX (для fee)\n\n"
        f"⚠ После ✅ деньги уйдут <b>сразу</b>, отмены не будет.",
        reply_markup=kb,
    )
...
async def cb_pay_cancel(call: CallbackQuery):
    if not is_owner(call.from_user.id):
        await call.answer("Только owner", show_alert=True); return
    pid = call.data.split(":", 1)[1]
    _pending_payouts.pop(pid, None)
...
async def cb_pay_confirm(call: CallbackQuery):
    if not is_owner(call.from_user.id):
        await call.answer("Только owner", show_alert=True); return
    pid = call.data.split(":", 1)[1]
    req = _pending_payouts.pop(pid, None)
    if not req:
        await call.answer("Запрос истёк или уже обработан", show_alert=True)
        return
    await call.answer("⏳ Отправляю в сеть...")
    try:
        await call.message.edit_text(
            f"⏳ <b>Отправляю</b> {req['amount_usdt']} USDT на <code>{req['to_address']}</code>...\n"
            f"<i>Подожди 30-60 сек для подтверждения в сети.</i>"
        )
    except Exception:
        pass
    ...
    try:
        ...
        result = await send_usdt_to(
            ...
            reason=f"{req['reason']} (@{req['worker_username']})",
            wait_confirmation=True,
            timeout_sec=90,
        )
    except Exception as e:
        logger.exception("[/payout] send failed: %s", e)
        await call.message.bot.send_message(
            call.message.chat.id, f"❌ <b>Ошибка отправки:</b>\n{str(e)[:300]}"
        )
        return
    ...
    if not result.get("ok"):
        await call.message.bot.send_message(
            call.message.chat.id,
            f"❌ <b>Выплата НЕ прошла:</b>\n{result.get('error', 'unknown')}"
        )
        return

    tx_hash = result.get("tx_hash", "")
    confirmed = result.get("confirmed", False)
    explorer = f"https://tronscan.org/#/transaction/{tx_hash}"
    status_emoji = "✅" if confirmed else "⏳"
    status_label = "ПОДТВЕРЖДЕНА" if confirmed else "Отправлена (ждём confirmation)"

    # Запись в accounting (salaries)
    try:
        await crm_storage.add_accounting_entry(
            category="salaries",
            amount_usdt=req["amount_usdt"],
            note=f"@{req['worker_username']} · {req['reason']} · tx:{tx_hash[:16]}...",
            created_by=f"auto:{call.from_user.username or call.from_user.id}",
        )
    except Exception as e:
        logger.warning("[/payout] accounting record failed: %s", e)

    # Notify owner (JARVIS bell + TG)
    try:
        await _notify_owner_tg(
            call.message.bot,
            f"💸 Выплата отправлена @{req['worker_username']}: {req['amount_usdt']} USDT\nTX: {tx_hash[:24]}...",
            dedup_key=f"payout_done:{tx_hash}",
        )
    except Exception:
        pass
    ...
    await call.message.bot.send_message(
        call.message.chat.id,
        f"{status_emoji} <b>Выплата {status_label}</b>\n\n"
        f"@{req['worker_username']}: <b>{req['amount_usdt']} USDT</b>\n"
        f"Адрес: <code>{req['to_address']}</code>\n"
        f"TX: <a href='{explorer}'>{tx_hash[:24]}...</a>\n\n"
        f"💼 Баланс после: <b>{bal_after.get('usdt', 0):.2f} USDT</b> | "
        f"{bal_after.get('trx', 0):.2f} TRX\n\n"
        f"📒 Записано в Бухгалтерию → Зарплаты",
        disable_web_page_preview=True,
    )


@router.message(Command("tron_balance"))
async def cmd_tron_balance(message: Message):
    """Owner: показывает баланс hot-wallet'а."""
    if not is_owner(message.from_user.id):
        return
    try:
