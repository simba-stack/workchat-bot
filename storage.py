...
class Storage:
    ...
    async def set_worker_usdt_address(self, username: str, address: str) -> bool:
        ...
        async with _lock:
            roles = self.state.setdefault("worker_roles", {})
            entry = roles.setdefault(clean, {"role": "", "is_admin": False})
            entry["usdt_address"] = (address or "").strip()
            await self._save_unlocked()
            return True

    def get_worker_usdt_address(self, username: str) -> str:
        clean = (username or "").lstrip("@").strip().lower()
        return ((self.state.get("worker_roles") or {}).get(clean) or {}).get("usdt_address") or ""
    ...
    # === NOTIFICATIONS QUEUE (alerts для owner / других ролей в JARVIS + TG) ===
    # Структура: state.notifications = [
    #   {id, ts, target_role, target_user_id, type, text, action_url, read}
    # ]
    # type: 'warning' | 'info' | 'success' | 'error'
    # target_role/user_id — для фильтрации (None = всем owner)

    async def add_notification(
        self, type: str, text: str,
        target_role: str = None, target_user_id: int = None,
        action_url: str = None, dedup_key: str = None,
    ) -> dict:
        """Добавляет уведомление в очередь. dedup_key — если такой ключ уже есть
        в последних 50 нотификациях, не дублируем."""
        import time
        async with _lock:
            notifs = self.state.setdefault("notifications", [])
            if dedup_key:
                for n in notifs[-50:]:
                    if n.get("dedup_key") == dedup_key and not n.get("read"):
                        return n  # уже есть unread с этим ключом
            seq = int(self.state.get("notifications_seq", 0)) + 1
            self.state["notifications_seq"] = seq
            entry = {
                "id": seq,
                "ts": time.time(),
                "type": type or "info",
            ...
            notifs.append(entry)
            # Не разрастаемся бесконечно — храним последние 500
            if len(notifs) > 500:
                self.state["notifications"] = notifs[-500:]
            await self._save_unlocked()
            return entry

    def list_notifications(
        self, role: str = None, user_id: int = None,
        since_ts: float = 0, unread_only: bool = False, limit: int = 100,
    ) -> list:
        """Возвращает нотификации видимые этому пользователю.
        Owner видит ВСЁ. Остальные — только свои target_role/user_id и broadcast."""
        notifs = self.state.get("notifications") or []
        out = []
        is_owner = (role == "owner")
        for n in notifs:
            if since_ts and n.get("ts", 0) <= since_ts:
                continue
            if unread_only and n.get("read"):
                continue
            # Filter visibility
            t_role = n.get("target_role")
            t_uid = n.get("target_user_id")
            if not is_owner:
                # Не owner — видит только если адресовано ему или роли
                if t_uid and user_id and int(t_uid) != int(user_id):
                    continue
                if t_role and t_role != role:
                    continue
                # broadcast (без таргета) — только owner видит
                if not t_uid and not t_role:
                    continue
            out.append(n)
        out.sort(key=lambda x: x.get("ts", 0), reverse=True)
        return out[:limit]

    async def mark_notification_read(self, notif_id: int) -> bool:
        async with _lock:
            for n in (self.state.get("notifications") or []):
                if int(n.get("id") or 0) == int(notif_id):
                    n["read"] = True
                    await self._save_unlocked()
                    return True
            return False

    # === TRON OUTBOUND LOG (исходящие USDT transfers) ===
    async def add_tron_outbound(
        self, tx_hash: str, to_address: str, amount_usdt: float,
        reason: str = "", status: str = "broadcasted",
    ) -> dict:
        import time
        async with _lock:
            log = self.state.setdefault("tron_outbound_log", [])
            entry = {
                "tx_hash": tx_hash,
                "to_address": to_address,
                "amount_usdt": float(amount_usdt),
                "reason": reason or "",
                "status": status,  # broadcasted / confirmed / failed
                "ts": time.time(),
            }
            log.append(entry)
            if len(log) > 1000:
                self.state["tron_outbound_log"] = log[-1000:]
            await self._save_unlocked()
            return entry

    async def update_tron_outbound(self, tx_hash: str, **fields) -> bool:
        async with _lock:
            for e in (self.state.get("tron_outbound_log") or []):
    ...
    def list_tron_outbound(self, limit: int = 100) -> list:
        log = self.state.get("tron_outbound_log") or []
        return list(reversed(log[-limit:]))

    # === AD CAMPAIGNS (рекламные кампании, выплаты менеджерам рекламодателей) ===
    async def add_ad_campaign(
        self, manager_username: str, amount_usdt: float, usdt_address: str = "",
        platform: str = "", note: str = "", date_start: str = "", date_end: str = "",
    ) -> dict:
        """Создаёт черновик кампании. status='draft'. Address можно ввести позже."""
        import time
        async with _lock:
            seq = int(self.state.get("ad_campaigns_seq", 0)) + 1
            self.state["ad_campaigns_seq"] = seq
            entry = {
                "id": seq,
                ...
                "status": "draft",   # draft → awaiting_address → awaiting_approval → approved → paid
                "tx_hash": "",
                "approved_by": "",
                "created_at": time.time(),
            }
            self.state.setdefault("ad_campaigns", []).append(entry)
            await self._save_unlocked()
            return entry

    async def update_ad_campaign(self, campaign_id: int, **fields) -> bool:
        async with _lock:
            for e in (self.state.get("ad_campaigns") or []):
                if int(e.get("id") or 0) == int(campaign_id):
                    for k, v in fields.items():
                        e[k] = v
                    await self._save_unlocked()
                    return True
    ...
    def list_ad_campaigns(self, status: str = None) -> list:
        out = list(self.state.get("ad_campaigns") or [])
        if status:
            out = [e for e in out if e.get("status") == status]
        return sorted(out, key=lambda x: x.get("created_at", 0), reverse=True)

    def get_ad_campaign(self, campaign_id: int) -> dict:
        for e in (self.state.get("ad_campaigns") or []):
            if int(e.get("id") or 0) == int(campaign_id):
                return dict(e)
        return {}

    async def mark_all_notifications_read(self, role: str = None, user_id: int = None) -> int:
        """Помечает все видимые нотификации как прочитанные. Возвращает кол-во."""
        async with _lock:
            count = 0
            is_owner = (role == "owner")
            for n in (self.state.get("notifications") or []):
    ...
    # ЖЕЛЕЗНАЯ КОНСТАНТА (SIMBA): PRIDE берёт ТОЛЬКО 3 банка.
    # Больше НИКАКИХ (ВТБ, Сбер, Тинькофф, Уралсиб, Точка, Локо, ПСБ, ...).
    DEFAULT_LK_PRICES = {
        "АЛЬФА": 400.0,
        "ОЗОН":  400.0,
        "РАЙФ":  400.0,
