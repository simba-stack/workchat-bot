"""Bot UI в стиле Crypto Bot: /start /wallet /p2p /checks /settings и callbacks.

Все команды отвечают inline-меню с кнопками (без открытия Mini-App для лишних
шагов). Mini-App используется только для одной кнопки «Открыть кошелёк».
"""
...
# ─── Эмодзи иконки монет (как в Crypto Bot) ──────────────────────────
COIN_ICONS = {
    "USDT": "💵", "USDC": "🔵", "TON": "💎", "TRX": "🔴",
    "BTC": "🟠", "ETH": "🟣", "SOL": "🟢", "BNB": "🟡",
    "DOGE": "🐕", "LTC": "Ł",  "XAUT": "🥇", "RUB": "₽",
...
async def cmd_start(message: Message):
    text = (
        "💎 <b>Мультивалютный криптокошелёк PRIDE P2P.</b>\n\n"
        "Покупайте, продавайте, храните, отправляйте и платите криптовалютой, когда хотите.\n\n"
        "Подписывайтесь на <a href=\"https://t.me/PrideP2P\">наш канал</a> "
        "и вступайте в <a href=\"https://t.me/PrideP2P_chat\">наш чат</a>."
    )
...
# ═══════════════════ /wallet — список балансов ══════════════════════
async def _build_wallet_text(tg_id: int) -> str:
    """Текст для /wallet — список монет с балансом и USD."""
    ...
    async with AsyncSessionLocal() as db:
        ...
        balances = await balance_service.list_balances(db, u.id)  # {USDT: Decimal, ...}
    ...
    for c in coins:
        ...
        if c.code in ("USDT", "USDC"):
            ...
            usd = bal * price
        total_usd += usd
        # Формат: показываем максимум 4 знака для крипты
        if bal == 0:
            line = f"{icon} <b>{c.name}</b>: 0 {c.code}"
        else:
            amt_str = f"{float(bal):.6f}".rstrip("0").rstrip(".") if bal < 1 else f"{float(bal):.4f}"
            usd_str = f" (${float(usd):.2f})" if usd > 0 else ""
            line = f"{icon} <b>{c.name}</b>: {amt_str} {c.code}{usd_str}"
        lines.append(line)
    lines.append(f"\n≈ <b>${float(total_usd):.2f}</b> общий баланс")
    return "\n".join(lines)
...
async def _show_p2p(target: Message, edit: bool = False):
    text = (
        ...
        "<a href=\"#\">Смотреть видеоинструкцию</a> ›"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_p2p_sub(call: CallbackQuery):
    ...
    if action == "buy":
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [_cb("Solana (SOL) · 9303₽ · 5", "p2p:coin:SOL:buy")],
            [_cb("TRON (TRX) · 36.88₽ · 8", "p2p:coin:TRX:buy")],
            [_cb("Bitcoin (BTC) · 7651834₽ · 3", "p2p:coin:BTC:buy")],
            [_cb("Ethereum (ETH) · 215882₽ · 3", "p2p:coin:ETH:buy")],
            [_cb("Dogecoin (DOGE) · 8.89₽ · 3", "p2p:coin:DOGE:buy")],
            [_cb("Litecoin (LTC) · 4000₽ · 6", "p2p:coin:LTC:buy")],
            [_cb("Binance Coin (BNB) · 162900₽ · 2", "p2p:coin:BNB:buy")],
        ...
        await call.message.edit_text(
            "⚙ <b>Оплата и валюта</b>\n\nЗдесь вы можете выбрать валюту отображаемых объявлений или управлять своими способами оплаты.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        name = (u.username and ("@" + u.username)) or (call.from_user.first_name or "Гость")
        await call.message.edit_text(
            f"👤 <b>{name}</b>\n\n"
            f"<a href=\"#\">Ваша статистика торговли</a> за <b>30 дней</b>:\n"
        ...
        await call.message.edit_text(
            f"Выберите способ оплаты для {'покупки' if side=='buy' else 'продажи'} <b>{coin}</b> за RUB.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                ...
                [_cb("Райффайзен Банк · 81₽ · 2", f"p2p:pm:Raif:{coin}:{side}")],
                [_cb(f"‹ Назад", "p2p:buy" if side=="buy" else "p2p:sell")],
            ]),
        )
    else:
        await call.answer(f"Раздел в разработке 🚀", show_alert=True)
        return
    await call.answer()
...
async def _show_checks(target: Message, edit: bool = False):
    text = (
        ...
        "используя виртуальные чеки. Забудьте про длинные адреса кошельков и ожидание подтверждений."
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_checks_sub(call: CallbackQuery):
    action = call.data.split(":", 1)[1]
    if action == "my":
        # Запрашиваем чеки из API
        try:
            from core.db import AsyncSessionLocal as _S
            ...
            async with _S() as db:
                ...
                rows = (await db.execute(
                    select(Cheque)
                    .where((Cheque.creator_user_id == u.id) | (Cheque.redeemed_by_user_id == u.id))
            ...
            if not rows:
                ...
                lines = ["📋 <b>Мои чеки</b>\n"]
                for c in rows[:15]:
                    icon = COIN_ICONS.get(c.coin_code, "🪙")
                    status = {"active": "✅", "redeemed": "💸", "cancelled": "❌"}.get(c.status, "•")
                    lines.append(f"{status} {icon} {float(c.amount)} {c.coin_code}")
                await call.message.edit_text(
                    "\n".join(lines),
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[[_cb("‹ Назад", "main:checks")]]),
                )
        ...
        await call.answer("Используйте inline-режим: @PrideP2P_bot <сумма> в чате", show_alert=True)
    await call.answer()
...
async def cmd_swap(message: Message):
    await message.answer(
        "🔄 <b>Обмен</b>\n\nБыстрый обмен между криптовалютами с моментальной фиксацией курса.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_swap_rates(call: CallbackQuery):
    try:
        ...
        for code, cg in (("USDT","tether"),("TON","the-open-network"),("TRX","tron"),
            ...
            arrow = "📈" if ch >= 0 else "📉"
            lines.append(f"{icon} <b>{code}</b> ${usd:.4f} {arrow} {ch:+.2f}%")
        await call.message.edit_text(
            "\n".join(lines),
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [_cb("‹ Назад", "main:swap")],
...
async def _show_settings(target: Message, tg_user_id: int | None = None, edit: bool = False):
    tg_id = tg_user_id or target.from_user.id if not edit else tg_user_id
    async with AsyncSessionLocal() as db:
        u = (await db.execute(select(User).where(User.tg_id == tg_id))).scalar_one_or_none()
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [_cb("ℹ Комиссии и лимиты", "set:fees")],
        [_cb("🎖 Конкурсы", "set:contests")],
...
async def cb_settings_sub(call: CallbackQuery):
    action = call.data.split(":", 1)[1]
    msgs = {
        "ref": "👥 <b>Рефералы</b>\n\nПриглашайте друзей и получайте % с их сделок.\nТвоя реф-ссылка: <code>https://t.me/PrideP2P_bot?start=ref_" + str(call.from_user.id) + "</code>",
        "notif": "🔔 <b>Уведомления</b>\n\n• Депозиты: ✅\n• Чеки: ✅\n• Сделки: ✅",
        "tz": "🌍 <b>Часовой пояс</b>: Europe/Moscow",
        "lang": "🇷🇺 <b>Язык</b>: Русский",
        "fiat": "💱 <b>Валюта отображения</b>: USD",
        "fees": "ℹ <b>Комиссии и лимиты</b>\n\n• Withdraw USDT TRC20: 3.5 USDT\n• Swap: 1%\n• P2P: 0.7%\n• Min withdraw USDT: 5",
        "contests": "🎖 Конкурсы пока не активны.",
        "help": "📖 <a href=\"https://t.me/PrideP2P\">Справка</a>",
        "about": "🤔 PRIDE P2P — мультивалютный кошелёк + P2P биржа в Telegram.",
