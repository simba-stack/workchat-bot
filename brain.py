...
def _build_system_prompt(brain_notes: str = "", client_context: Optional[dict] = None) -> str:
    ...
    intro = (
        ...
        "Если клиент говорит про сделку («сегодня пополнят», «когда выплата», "
        "«статус ЛК?», «доработали?», «когда заберём?», «уже пополнено?»):\n"
        "  1. Обязательно уточни номер сделки цифрами (если ещё не знаешь).\n"
        "  2. Вызови инструмент `find_deal(deal_id=<номер>)` или "
        "     `find_deal(username=<никнейм>)`.\n"
        "  3. Отвечай СТРОГО тем что вернул инструмент (bank, status, price, "
        "     usdt_address). Если статус «done/выплачено» — говори как есть. "
        "     Если «в работе» — так и говори. Если сделка не найдена — прямо "
        "     скажи «в системе не нахожу, уточним у бухгалтера» и вызови "
        "     `escalate_to_team(specialist='accounting', ...)`.\n"
        "  4. НИКОГДА не пиши «Понял, начинаем перевязку», «Сделка пополнена, "
        "     завтра стартуем», «Оплата уже ушла», «Всё готово» и подобное — "
        "     если этого нет в данных find_deal, это ложь клиенту.\n\n"
