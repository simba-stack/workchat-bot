...
class UserbotService:
    ...
    async def _handle_track_choice(self, event) -> bool:
        ...
        # AUDIT #5 H-NEW-1 (авг 2026): также ловим прямые названия отделов
        # (бухгалтер/менеджер/сус) в сочетании с глаголами вызова —
        # «позовите бухгалтера» / «нужен менеджер» / «дайте суса».
        _call_op_re = re.search(
            ...
            # H-NEW-1: прямые названия отделов с глаголом вызова
            r"(?:позов|зов|дай|нужен|нужн[аы]|хочу|позвать|свяж|подключ)"
            r"\w*\W+(?:бухгалтер\w*|менеджер\w*|сус\w*)|"
            r"(?:бухгалтер\w*|менеджер\w*|сус\w*)\W+(?:нужен|нужн[аы]|срочно|плиз|пожалуйста)",
            text, re.IGNORECASE,
        )
        if _call_op_re:
        ...
        # снятия awaiting_track_choice — клиент дальше выбирает 1/2).
        _price_ask = re.search(
            r"^\s*прайс\w*\s*[?.!]*\s*$|"
            r"^\s*цен[ыаоуе]\w*\s*[?.!]*\s*$|"
            r"\bпрайс\w*\b|"
            r"какие\s+цен|"
        ...
        if _price_ask:
            ...
            logger.info(
                "[welcome] price-ask shortcut chat=%s text=%r → reply_pricing",
                chat_id, text[:60],
            )
            await self._send_scripted(
                chat_id, "reply_pricing_generic",
                default_text=f"Актуальные цены:\n\n{pricing_text}\n\n"
                             f"Если интересует конкретный банк — напишите название.\n\n"
                             f"Продолжаем оформление? Напишите цифру 1 (ИП/ООО) или 2 (Дебет).",
                pricing=pricing_text,
            )
            return True  # handled, awaiting остаётся
    ...
    async def _handle_ai_message(self, event):
        ...
        try:
            ...
            # AUDIT #3 HIGH-7 (авг 2026): «готов/готова/готовы» + «могу»
        ...
        try:
            ...
            if _sf_step and _sf_step not in ("done", "cancelled", "") and not _sf_stale:
                # AUDIT #6 H-B2 + #9 H9-1 (авг 2026): текстовая «отмена» на
                # КНОПОЧНЫХ экранах (material/bank/payment/verification_pending —
                # нет FSM state, HIGH-8 глушит). ИСКЛЮЧАЕМ степы где уже есть
                # свой FSM-cancel в sell_wizard_crm.py — иначе клиент получает
                # 2× «Оформление отменено» от userbot + crm_bot.
                # FSM-owned степы: verification_upload (waiting_screenshot/
                # video/inn/upload), guarantor_wait_deal_number.
                _FSM_OWNED_STEPS = {"verification_upload", "guarantor_wait_deal_number"}
                if _sf_step not in _FSM_OWNED_STEPS:
        ...
        if not storage.is_ai_enabled():
            ...
            try:
                ...
                if _low and re.search(
                    ...
                    r"(?:^|\s)с\s+(?:альф\w*|озон\w*|райф\w*|райффайз\w*)\s+"
                    r"(?:вс[её]|ок|всё\s+ок|порядок|готово|отработ\w*)\s*[?!.]?\s*$|"
                    r"^\s*что\s+(?:дальше|нужно|делать|делаем|нам\s+делать)\s*[?!.]?\s*$|"
                    r"^\s*приступа(?:ем|ешь|ть)\s*[?!.]?\s*$|"
                    r"^\s*дальше\s+что\s*[?!.]?\s*$|"
    ...
    async def _try_faq_scripted_reply(self, event, chat_id, text: str) -> bool:
        ...
        # AUDIT #5 H-NEW-1: также прямые названия отделов с глаголом вызова.
        if re.search(
            r"(?:^|\W)асик\W*[,]?\W*позов(?:и|ите)\W+оператор|"
            r"позов(?:и|ите)\W+оператор|"
            r"зов(?:и|ите)\W+оператор|"
            ...
            r"живы[йех]\W+оператор|"
            r"живые\W+операторы|"
            r"^\s*асик[,.!?]?\s+оператор\b|"
            r"^\s*оператор\W+(?:нужен|срочно|плиз|пожалуйста)|"
            r"\bгде\W+оператор|"
            r"\bнайти\W+оператор|"
            r"свяж(?:ите|ешь)\W+с\W+оператор|"
            r"подключ(?:ите|и)\W+оператор|"
        ...
        if _WIZARD_TRIGGER.search(low):
            try:
                await storage.enqueue_dashboard_command(
                    f"__start_sell_wizard chat={int(chat_id)}",
                    source="asik_work_intent",
                )
                logger.info(
                    "[sell_wizard_crm] HARD-trigger (intent) → start_wizard chat=%s text=%r",
                    chat_id, low[:80],
                )
            except Exception as _e:
        ...
        # #2 reply_payment_options — УДАЛЁН (SIMBA HARD RULE v3):
        # вопросы «как оплата?» → идут в брейн-fallback (админ отвечает).
        # #3 reply_payment_usdt_hint — УДАЛЁН: USDT/адрес flow ведёт CRM-визард.
        ...
        # AUDIT #3 Cat-12 (авг 2026): действия с конкретной сделкой N.
        # «пополни сделку 12345», «отпусти сделку 12345», «отмена сделки»,
        # «сколько на сделке 12345», «статус сделки 12345» — ловим номер
        # + форвардим в бухгалтерский dept-чат. Раньше эти фразы уходили в
        # общий reply_payment_status (статичный текст «выплаты по условиям»)
        # без учёта номера — бухгалтерия не получала запрос.
        # AUDIT #5 H-NEW-2 (авг 2026): «где деньги по сделке N» — раньше не
        # ловилось из-за «по» между глаголом и «сделка». Добавили гибкий буфер
        # 0-2 слова между act и «сделк».
        _deal_action = re.search(
            r"(?P<act>пополн\w*|отпуст\w*|отмен\w*|сколько|статус|где\s+деньг\w*)"
            r"(?:\s+\S+){0,3}?\s+"
            r"(?:на\s+|по\s+)?сделк\w*\W*[#№]?\s*(?P<num>\d{3,10})?",
            low, re.IGNORECASE,
        )
        if not _deal_action:
            # Второй вариант — «сделка 12345 пополнить» и т.п.
            _deal_action = re.search(
                r"сделк\w*\W*[#№]?\s*(?P<num>\d{3,10})\W+"
                r"(?P<act>пополн\w*|отпуст\w*|отмен\w*|сколько|статус)",
                low, re.IGNORECASE,
            )
        if _deal_action:
            _act_raw = _deal_action.group("act")
            _num = _deal_action.group("num") or "?"
            _act_map = {
                "пополн": "пополнение",
                "отпуст": "отпуск/освобождение",
                "отмен": "отмена",
                "сколько": "запрос баланса",
                "статус": "запрос статуса",
                "где": "запрос статуса",
            }
            _act_label = "запрос по сделке"
            for _k, _v in _act_map.items():
                if _k in _act_raw.lower():
                    _act_label = _v
                    break
            # Форвардим в бухгалтерию (если настроена)
            try:
                _acc_id = storage.get_accounting_dept_chat_id()
                if _acc_id:
                    _ci = storage.get_chat_info(chat_id) or {}
                    _cli = _ci.get("client_username") or _ci.get("client_id") or "?"
                    _fwd_text = (
                        f"💰 <b>Запрос по сделке #{_num}</b>\n"
                        f"Действие: <b>{_act_label}</b>\n"
                        f"Клиент: @{_cli}\n"
                        f"Чат: <code>{chat_id}</code>\n\n"
                        f"Исходное сообщение:\n<i>{(text or '')[:400]}</i>"
                    )
                    try:
                        await self.client.send_message(_acc_id, _fwd_text)
                        logger.info(
                            "[cat12] deal action fwd: chat=%s deal=%s act=%s → acc=%s",
                            chat_id, _num, _act_label, _acc_id,
                        )
                    except Exception as _fe:
                        logger.warning("[cat12] fwd to accounting failed: %s", _fe)
                else:
                    logger.warning(
                        "[cat12] no accounting_dept_chat_id — запрос по сделке #%s не форварднут",
                        _num,
                    )
            except Exception as _ae:
                logger.warning("[cat12] accounting lookup failed: %s", _ae)
            return await self._send_scripted(
                chat_id, "reply_deal_action_ack",
                default_text=(
                    f"✅ Запрос по сделке <b>#{_num}</b> ({_act_label}) "
                    f"передан в бухгалтерию.\n\n"
                    f"Ответят в ближайшее время. Если срочно — напишите:\n"
                    f"<code>Асик, позови оператора</code>"
                ),
            )
        ...
        # админ отвечает вручную. Единственный старт визарда — фраза
        # «Ассистент, хочу сдать РС» (см. _WIZARD_TRIGGER выше).
        #
        # #8 reply_traffic_volume — УДАЛЁН: вопросы про трафик/объём →
        # в брейн (админ уточняет по обстоятельствам).
        #
        # #9 reply_when_take — УДАЛЁН: сроки/готовность → в брейн.
        #
        # #10 старый «готов/давайте/поехали» триггер визарда — УДАЛЁН:
        # заменён строгим _WIZARD_TRIGGER в самом начале функции.
        #
        # #11 ask_continental_deal_number — УДАЛЁН: гарант-flow ведёт
        # CRM-визард (sell_wizard_crm.py). Асик больше НЕ должен слать
        # «Создайте сделку в @PRIDE_BUHGALTERIA» — это конкурировало с
        # визардом и путало клиента.
        #
        # #12 USDT-shortcut — УДАЛЁН: USDT-выбор внутри визарда, не текстом.
    ...
    async def _create_lk_card_from_perevyaz(
        ...
        # SIMBA (авг 2026 — batch M FIX): УДАЛЕНО создание карточки в АУДИТ
        # отсюда. Раньше _tool_create_lk_card триггерил _push_audit_bot →
        # карточка появлялась в АУДИТ-группе ДО того как СУС отметил перевязку
        # завершённой. Клиент MAVRODI SKUP: карточка #83 создалась в 14:59,
        # а «Предоставьте СМС-код для перевязки» пришло в 15:08 — то есть
        # карточка появилась ДО перевяза.
        #
        # Теперь карточка в АУДИТ создаётся ТОЛЬКО из cb_perevyaz когда
        # sms_stage == "perevyaz_received" (crm_bot.py _create_single_lk_card).
        # Здесь оставляем только локальную lk_card (storage.add_lk_card выше),
        # она без inline-карточки в АУДИТ.
