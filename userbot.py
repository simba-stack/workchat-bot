...
_FORBIDDEN_CLIENT_PATTERNS = [
    ...
    r"\bпополните\s+(?:сделку|сделки|обе|её|их)",
    r"\bвнесите\s+(?:средства|деньг\w+|сумм\w+)\s+(?:в\s+)?гарант",
    r"\bпополнить\s+обе\s+сделки",
    r"\bвам\s+нужно\s+пополнить\s+сделк",
    r"\bпосле\s+пополнения\s+вами",
    r"\bкак\s+пополните\s+сделк",
    r"\bкогда\s+пополните\s+сделк",
    r"\bждём\s+(?:вашего\s+)?подтверждения\s+пополнения",
