...
# ВАЖНО: memories/* НЕ загружается в brain (см. brain.py _SKIP_NAMES) —
# поэтому всё что должно влиять на ответы AI должно идти в файлы из root.
# Если фактически только memories/auto.md, AI не увидит саморожденное знание.
KNOWN_FILES = {
    "about.md": "О компании: что делает, ключевые факты, контакты, реквизиты",
    "pricing.md": "Цены, прайс банков, ставки, доплаты, скидки, депозит, холд",
    "faq.md": "Частые вопросы клиентов и готовые ответы (как отвечать)",
    "policy.md": "Что AI НЕ должен говорить, ограничения, запреты, hard rules",
    "style.md": "Тон общения, манера речи, шаблоны фраз",
    "deals.md": (
        "Процедура оформления сделок, статусы (ПОПОЛНИТЬ/В_РАБОТЕ/ЗАВЕРШЕНА), "
        "форматы сообщений в группах «Сделки и выплаты» / «Отработка аккаунтов», "
        "запросы списков, перевяз ЛК, гарант-сделки, всё что связано с deals."
    ),
    "memories/auto.md": (
        "ТОЛЬКО для исторических заметок, разовых эпизодов, низкоуровневых "
        "напоминаний. AI этот файл НЕ читает. Если факт должен влиять на "
        "будущие ответы — выбери один из root-файлов выше."
    ),
}
