...
def _classifier_prompt(text: str) -> str:
    ...
    return (
        ...
        "ОБЯЗАТЕЛЬНО skip (верни {\"skip\": true, \"reason\": \"...\"}):\n"
        "- Любой вопрос (заканчивается на ?, или начинается с «как», «что», «где», "
        "«можно», «а если», «правда ли», «ты», «вы»)\n"
        "- Приветствия, прощания, мета: «привет», «здарова», «ты тут», «ау», "
        "«проверка», «тест», «1», «ок», «угу», «хм», «попробую», «ща», «сек»\n"
        "- Эмоции и реакции: «класс», «огонь», «👍», «🤔», «лол»\n"
        "- Команды AI (ничему не учат): «забудь это», «отвечай короче», «не сохраняй»\n"
        "- Сообщения короче 20 символов (если не явный фактоид типа «телефон +7...»)\n"
        "- Сообщения, дублирующие то, что уже могло быть сохранено\n\n"
        "Сохраняй (status ok):\n"
        "- «Минимальный заказ — 10 000 ₽»\n"
        "- «Доставка по Москве: 500 ₽, 1-2 дня»\n"
        "- «Менеджер по поставкам — Алексей, @username»\n"
        "- «Работаем пн-пт с 9:00 до 18:00, сб-вс выходные»\n"
        "- «Скидку 5% даём при заказе от 50 000 ₽»\n\n"
